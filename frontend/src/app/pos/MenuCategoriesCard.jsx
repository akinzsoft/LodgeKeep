...
export function MenuCategoriesCard({ categories, onChanged }) {
  ...
  async function handleCreate(event) {
    ...
    setError(null);
    try {
      await posApi.createMenuCategory({ name: form.name, sortOrder: form.sort_order === '' ? undefined : Number(form.sort_order) });
  ...
  async function handleSaveEdit(event) {
    ...
    setError(null);
    try {
      await posApi.updateMenuCategory(editing.id, { name: editing.name, sortOrder: editing.sort_order === '' ? undefined : Number(editing.sort_order) });
      setEditing(null);
      await onChanged();
    } catch (caught) {
  ...
  async function confirmArchive() {
    ...
    setError(null);
    try {
      await posApi.archiveMenuCategory(category.id);
      await onChanged();
    } catch (caught) {
  ...
  return (
    <Card title="Menu categories">
      <p className={formStyles.hint}>Categories are shared by every outlet. Menu items choose one of these, so the Register and guest menu show consistent names.</p>
      {error && (
      ...
      <form className={formStyles.row} onSubmit={handleCreate}>
        <label className={formStyles.field}>
          ...
          <input className={formStyles.input} value={form.name} maxLength={60} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Starters" required />
        </label>
        <label className={formStyles.field}>
      ...
      <DataTable
        ...
        columns={[
          ...
          { key: 'item_count', label: 'Menu items', align: 'right' },
        ]}
        rows={categories ?? []}
        rowKey={(row) => row.id}
        actions={(row) => (
      ...
      {editing && (
        <form className={formStyles.row} onSubmit={handleSaveEdit} aria-label="Edit category">
          ...
          <p className={formStyles.hint}>Renaming updates every menu item in this category.</p>
        </form>
      )}
      ...
      {archiving && (
        <ConfirmDialog
          ...
          consequence={`"${archiving.name}" will no longer be offered for menu items. A category still used by menu items cannot be archived — move those items first.`}
