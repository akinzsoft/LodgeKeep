...
- **Data migration (3.20)** — import templates, dry run, dedupe, rollback — ✅ **shipped**: a new `src/modules/migration` module — real multipart CSV upload (this codebase's first file-upload endpoint) for guests/reservations/companies/AR opening balances, one column template per entity, a synchronous re-runnable dry run (row counts, per-row error/duplicate/conflict table, nothing written until commit), guest dedup in email → phone → name+date-of-birth order with a `use_existing`/`create_new` resolution per candidate blocking commit until resolved (never auto-merged), historical reservations bypassing the live overbooking lock while future-dated conflicts surface as warnings and still commit once confirmed, a job-based per-row commit (a sixth, one-off `imports` BullMQ queue) where one row's failure is logged and skipped without aborting the run, and rollback that deletes what's still untouched and refuses+reports the rest as an honest partial rollback.
...
- Migration: dry run writes nothing, duplicate detection surfaces matches without auto-merging, rollback fully reverses a run — ✅ covered (`tests/migration/migration.test.js`, `tests/migration/rollback.test.js`, `tests/migration/dedup.test.js`, `tests/migration/validate.test.js`)
