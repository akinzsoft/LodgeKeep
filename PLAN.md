...
- **Plans and entitlements** — the single entitlement check, applied to already-built features — 🔲 not built. A real `plans` catalogue now exists (one `standard` plan, `tenants.plan_id` wired up and set on trial conversion), but no feature in this codebase is gated by which plan a tenant is on — every plan grants the same access. Entitlement gating itself remains this phase's own next, explicitly deferred piece.
...
**Tests required to close:**
- Entitlement gating enforced at the API, not just hidden in the UI — a tenant on a lower plan calling a gated endpoint directly is rejected — 🔲 not applicable yet (a real plan catalogue now exists, but every plan grants identical access — no gate to test)
