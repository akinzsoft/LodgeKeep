...
**Platform staff RBAC** (PLAN.md Phase 5) — this matrix is entirely tenant-role-shaped and deliberately has no row for platform staff, a genuinely separate population (§4's role model). This session's confirmed decision: `platform_users` carries no role/permission concept at all in this pass — any active, MFA-verified platform account can list tenants and start an impersonation grant, the same "internal, trusted-engineer tool" framing this population has had since Phase 0. Real platform-side RBAC (e.g. a support agent who can impersonate but not later suspend a tenant or touch billing, once those actions exist) is a genuine, separate, flaggable follow-on once there is more than one class of platform employee — not inferred or half-built here.
