...
- **A webhook is a hint; the provider's own record is the truth.** A valid signature proves who SENT an event, not that what it claims is what the provider holds. After persisting a signed event, the handler asks the provider for its record of the transaction (Verify Transaction), compares reference, amount and currency to the LOCAL payment (`src/shared/gateway-record.js`, exact integers, fail-closed on a missing amount), and applies only the outcome the record dictates — never status, amount or currency taken from the webhook body. A disagreement is recorded (`outcome = 'rejected'`) and audited; nothing is captured and the payment is left open. The verify call runs with no database transaction open, and an event whose verification is unavailable (timeout, rate limit, bad key) or whose transaction is not yet final is retried by a sweep, never decided. The browser/manual confirmation path (`verifyPayment`) applies the same check.
...
**The one exception: the retention purge.** Immutability governs what the application does to a *live* tenant's ledger. A tenant that has been offboarded, has been given a complete export of its data, and whose retention window has expired is deleted whole — every folio, payment, line item and the audit trail included — by `src/modules/offboarding/purge.js`. This is the only code path that ever deletes a financial row, it operates on a tenant's entire dataset at once (never a single record), it requires a completed export made after the offboarding request, and it is off unless `TENANT_PURGE_ENABLED=true`. Planmsys's own billing records for the tenant (`subscriptions`, `subscription_invoices`, `subscription_payments`) and a `tenants` tombstone are kept. Whether a jurisdiction's tax-record retention rules allow deleting a hotel's own ledger at all is a legal question this document does not answer; the switch stays off until it is answered.
