...
| `company_profiles` | tenant_id, name, type (company/travel_agent/source), billing_email, billing_phone, billing_address, payment_terms_days, status (active/archived) | Built, PLAN.md Phase 4 — **TENANT_SCOPED**, not property-scoped, the same "one identity across every property the tenant runs" reasoning `guests` already established (a company does business with more than one property in a tenant). `credit_limit`/`current_balance` live on `ar_accounts` instead (Cashiering & AR, below), not here — a company can hold a *separate* AR account, and a separate credit limit, per property (two properties in the same tenant run entirely separate ledgers, the same reasoning every other money-bearing table in this schema is PROPERTY_SCOPED). CRUD lives in the Profiles module (reads: `reservations.view`; writes: `ar.manage`, SECURITY.md §5) since these fields exist primarily to serve AR credit decisions. `guests.company_profile_id` (nullable FK, added alongside this table) links a guest to their employer/agent for display, set via a dedicated `POST /guests/:id/link-company` endpoint, not a general guest-update route (none exists yet) |
...
| Table | Key columns | Notes |
|---|---|---|
| `reservations` | guest_id, group_block_id, rate_code_id, market_segment_id, booking_source_id, arrival_date, departure_date, adults, children, status, confirmation_number, checked_in_at, checked_out_at, cancellation_policy_id, preferred_room_id | `guest_id` references the tenant-level `guests` row regardless of which property this reservation is for — the guest's identity doesn't change per property. status: waitlisted/confirmed/checked_in/checked_out/cancelled/no_show. `preferred_room_id` (gap closure) is a guest-requested room number recorded at booking time — a REQUEST, never a lock: `checkIn` still accepts any room, and this column is never checked against current occupancy (a preference for a future date can't honestly be). Only checked against the reservation's own `room_type_id` at booking time. See `room-availability.js`'s header for why "actual free room numbers" is otherwise only answerable for the property's current business date, never a future one. |
...
| `group_blocks` | tenant_id, property_id, company_profile_id, block_name, start_date, end_date, cutoff_date, status (active/cancelled), notes | Built, PLAN.md Phase 4. **PROPERTY_SCOPED**, following `ar_accounts`/`room_types` — a block is negotiated against one property's own inventory, not shared across a tenant's properties. No `rooms_blocked` total on this row (the original draft's column) — that would be redundant with the sum of `group_block_rooms.rooms_blocked` below, a second source of truth that could drift. Tracking-only (this session's confirmed decision): creating a block never withdraws inventory from `room_type_inventory` — a reservation tagged with `group_block_id` books through the completely unmodified last-room-race mechanism. `company_profile_id` is nullable — an optional AR sponsor; when set, the block's rooming list can be bulk-billed to that company's existing `ar_accounts` row (`billBlockReservationsToSponsor`, reusing `cashiering.billFolioToCompany` verbatim, zero new billing logic); unsponsored, each reservation in the block settles its own folio individually, unchanged. `cutoff_date` is informational only — nothing automated reads it, since there is no inventory hold to release in the first place. `status: cancelled` is the one enforcement point: `createReservation` refuses to tag a new reservation to a cancelled block |
| `group_block_rooms` | tenant_id, property_id, group_block_id, room_type_id, stay_date, rooms_blocked | Built, PLAN.md Phase 4. **No `rooms_picked_up` column**, unlike the original draft — pickup is always computed live (`group-blocks/pickup.js`'s `computePickupRows`, from real `reservations`/`reservation_daily_rates` rows tagged with the block's id), never a second, independently-maintained running total that could drift from what was actually booked — the identical "one source of truth, always re-derived" discipline `ar_accounts.current_balance`/`folios.balance` already established. Drives the pickup progress bar (PRODUCT_REQUIREMENTS.md, Back-office screens) |
...
**Cashiering & AR (3.5, 3.9)** — PLAN.md Phase 2.5 built the real ledger and gateway rows below (`folios` through `currency_rates`); Accounts Receivable (`ar_accounts` through `ar_payment_applications`) is real §3.9 scope, deferred at the time this section was first written but now built in a later Phase 4 pass — see that pass's own rows below, and CLAUDE.md's own "Phase 4 — Accounts Receivable" status section for the full build.

| Table | Key columns | Notes |
|---|---|---|
| `folios` | reservation_id, folio_number, billed_to, company_profile_id, currency, status | Built. `billed_to` stays a free-text display label even now that `company_profile_id` (nullable FK → `company_profiles`, added by the AR pass) is real — set automatically to the company's own name the moment a folio is billed to an account (`billFolioToCompany`), so the two never disagree. A folio with `company_profile_id` set may carry any balance at checkout (ARCHITECTURE.md §11's "or the property permits checkout with balance owing to AR" hook, implemented by this pass) and cannot accept a direct cash/card payment — settlement goes through `ar_payments` instead. Multiple folios per reservation for split billing (`openAdditionalFolio`) |
...
| `ar_accounts` | tenant_id, property_id, company_profile_id, credit_limit, currency, enforcement_mode (block/flag_only), current_balance, is_over_limit, status | Built, PLAN.md Phase 4. **PROPERTY_SCOPED**, one account per (company, property) — `UNIQUE(tenant_id, property_id, company_profile_id)`, not the tenant-wide single row the original draft column list implied (see `company_profiles`' own note above for why). `current_balance` is never an independently-updated running total — `recomputeArAccountBalance` is the only writer, always re-deriving it as the sum of every non-voided charge-type `folio_line_items` row on a folio billed to this account, minus every non-voided `ar_payments.amount`. `enforcement_mode` is this session's confirmed "configurable per account" resolution of TESTING.md AR-3's own "blocked or flagged per config": `block` rejects an over-limit charge (`422 BUSINESS_RULE_CREDIT_LIMIT_EXCEEDED`, API.md's own reserved code) unless a manager supplies an explicit override + reason; `flag_only` always allows the charge and only sets `is_over_limit` |
| `ar_invoice_sequences` | tenant_id, property_id, next_number | Built, PLAN.md Phase 4. Not in the original draft — a small per-property counter table backing a human-readable, sequential `invoice_number` (`INV-{propertyId}-{next_number}`), locked and incremented via the identical insert-if-missing-then-`SELECT ... FOR UPDATE`-then-increment shape `room_type_inventory`'s own last-room race already established, applied here to a different resource |
| `ar_invoices` | tenant_id, property_id, ar_account_id, invoice_number, currency, total_amount, status (issued/partially_paid/paid/void), issued_at, due_at, business_date, created_by_user_id, voided_at, voided_by_user_id, void_reason | Built, PLAN.md Phase 4. No `draft` status — generation directly produces `issued`, nothing in the confirmed scope asked for a review step. `overdue` is never stored — it's a label the ageing report derives at read time from `due_at` vs. the property's own `current_business_date` (ARCHITECTURE.md §6), never wall-clock. Void, never delete (ARCHITECTURE.md §8); voiding does NOT release the invoice's own lines back to "un-invoiced" — a genuine correction to an already-invoiced charge goes through a fresh offsetting `postAdjustment` on the folio instead, picked up by the next invoice run as its own new line |
| `ar_invoice_lines` | tenant_id, property_id, ar_invoice_id, folio_line_item_id, amount, currency, business_date | Built, PLAN.md Phase 4. Links to an individual `folio_line_items` row, not a whole folio (the original draft's simpler shape) — lets one invoice span multiple folios/reservations for the same company, and `UNIQUE(tenant_id, property_id, folio_line_item_id)` is the structural, database-enforced guarantee that a folio line is invoiced at most once, ever (TESTING.md AR-1's "total matches source folio lines" proof), on top of the primary "already-invoiced lines are excluded" query filter |
| `ar_payments` | tenant_id, property_id, ar_account_id, amount, currency, method_label, reference, received_at, business_date, recorded_by_user_id, notes, voided_at, voided_by_user_id, void_reason | Built, PLAN.md Phase 4. Manual recording only, this session's confirmed decision — no gateway integration and deliberately not the `payments` state machine above, which is folio-shaped (one payment per folio) where a real wire/cheque routinely covers several invoices across several folios at once. The full `amount` reduces `ar_accounts.current_balance` the moment it's recorded, regardless of how much of it is later *applied* to a specific invoice (see `ar_payment_applications`) |
| `ar_payment_applications` | tenant_id, property_id, ar_payment_id, ar_invoice_id, amount, voided_at, voided_by_user_id | Built, PLAN.md Phase 4. Join table, not the original draft's single `applied_to_invoice_id` column on `ar_payments` — required so one payment can be split across several invoices. Deliberately carries no uniqueness constraint on the (payment, invoice) pair: applying the same pair more than once (a payment applied in two installments, or a corrected re-application after voiding an earlier one) is a normal, legitimate sequence, not a duplicate — each application is its own event row, matching this schema's void-never-delete/one-row-per-event discipline. Bookkeeping allocation only (which invoice shows `partially_paid`/`paid`) — never a second, disagreeing notion of how much the company really owes, which is always `ar_accounts.current_balance` alone |
...
| `company_profiles` | active → archived | AR history (accounts/invoices/payments) references them regardless of status — archiving hides a company from new-billing pickers, never its past records |
